...
- **STM32F411CEUx** (theo tên file startup [startup_stm32f411ceux.s](MainECU_STM32/Core/Startup/startup_stm32f411ceux.s)) — ARM Cortex-M4F, 512KB Flash, 128KB RAM. Package UFQFPN48 (kiểu board "Black Pill").
- Firmware dùng **STM32 HAL** (stm32f4xx), khung project sinh bởi STM32CubeMX/CubeIDE.

## 2. Cấu hình Clock

Nguồn: [Wrappers_Clock.c](MainECU_STM32/Wrappers/Wrappers_Clock/Wrappers_Clock.c)
...
Nguồn: [main.h](MainECU_STM32/Core/Inc/main.h) và [stm32f4xx_hal_msp.c](MainECU_STM32/Core/Src/stm32f4xx_hal_msp.c)
...
### SPI ([Wrappers_Spi.c](MainECU_STM32/Wrappers/Wrappers_Spi/Wrappers_Spi.c))
...
### USART1 — link ESP8266 Station ([Wrappers_Uart.c](MainECU_STM32/Wrappers/Wrappers_Uart/Wrappers_Uart.c))
...
### TIM1 — HAL timebase
- Từ lần gen CubeMX 2026-08-28, tick HAL (`HAL_GetTick/HAL_Delay`) chạy bằng **TIM1** ([stm32f4xx_hal_timebase_tim.c](MainECU_STM32/Core/Src/stm32f4xx_hal_timebase_tim.c), IRQ `TIM1_UP_TIM10_IRQHandler`), không còn dùng SysTick. Code trong `SysTick_Handler` (FatFsCnt/Timer1/Timer2) hiện **không chạy nữa** — không sao vì driver SD không dùng chúng.

### TIM2 ([Wrappers_Pwm.c](MainECU_STM32/Wrappers/Wrappers_Pwm/Wrappers_Pwm.c))
...
### GPIO ([Wrappers_Gpio.c](MainECU_STM32/Wrappers/Wrappers_Gpio/Wrappers_Gpio.c))
- Enable clock port A/B/C/H; **cả 3 chân CS (Screen, Touch, SDCard) khởi tạo SET** (active-low, nhả bus — mỗi driver tự kéo CS khi giao dịch); các chân output còn lại (RST/DC/backlight/PC13) khởi tạo RESET.
- Đọc/ghi qua macro `Wrappers_Gpio_Write(LogicalChannel_x, level)` / `Wrappers_Gpio_Read(LogicalChannel_x)`.

### Cảm ứng XPT2046 ([Libs_XPT2046_Driver.c](MainECU_STM32/Libs/Libs_Touch_XPT2046/Libs_XPT2046_Driver.c))
...
- Header [Libs_ILI9341_Driver.h](MainECU_STM32/Libs/Libs_Screen_ILI9341/Libs_ILI9341_Driver.h) ghi chú benchmark: với SPI 50MHz đạt tối đa lý thuyết ~40 FPS full-screen; hiện SPI1 chạy 12.5 Mbit/s nên FPS thực tế thấp hơn (~10 FPS full-screen lý thuyết).
...
- Middleware: FatFs (bản đi kèm CubeMX) tại [Middlewares/Third_Party/FatFs/](MainECU_STM32/Middlewares/Third_Party/FatFs/src/) — **code third-party, không sửa trực tiếp**.
- Glue code: [FATFS/App/fatfs.c](MainECU_STM32/FATFS/App/fatfs.c) (link driver `USER_Driver`, `get_fattime()` trả 0 vì chưa có RTC) và [FATFS/Target/user_diskio.c](MainECU_STM32/FATFS/Target/user_diskio.c).
- **Trạng thái: hoạt động.** [Libs_SdCard](MainECU_STM32/Libs/Libs_SdCard/Libs_SdCard.c) implement giao thức SD-SPI (CMD0/CMD8/ACMD41/CMD58; hỗ trợ SDv1/SDv2/SDHC/MMC, nhận diện block-addressing qua bit CCS trong OCR); `user_diskio.c` chỉ là glue gọi xuống driver trong các block `USER CODE`.
...
Nguồn: [stm32f4xx_it.c](MainECU_STM32/Core/Src/stm32f4xx_it.c)
...
- Mọi lời gọi `HAL_xxx_Init` đều kiểm tra `!= HAL_OK` → gọi `Error_Handler()` ([main.c](MainECU_STM32/Core/Src/main.c)): tắt ngắt (`__disable_irq`) và dừng trong vòng lặp vô hạn.
