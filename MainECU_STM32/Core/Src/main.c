...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define LED_TOGGLE_PERIOD_MS            500u    // toggle 500ms -> chu ky nhay 1s
/* USER CODE END PD */
...
/* USER CODE BEGIN PV */
static uint32_t l_LedToggleAtMs_u32 = 0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  /* Peripheral (SPI/UART/TIM/GPIO) da duoc MX_*_Init phia tren lo.
     Tu day tro di: chi lam viec qua tang Libs/App theo convention (Doc/03). */
  Libs_ILI9341_Init();
  Libs_XPT2046_Init();
  ...
  while (1)
  {
    ...
    {
      l_LedToggleAtMs_u32 = Libs_System_GetTick();
      Libs_Gpio_Toggle(LogicalChannel_8);
    }
  }
  /* USER CODE END 3 */
}
