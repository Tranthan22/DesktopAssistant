...
│   └── Libs_EspLink/      #   Nhận/trả lệnh từ ESP Station qua UART (kèm bản sao StationLinkCore)
├── App/                   # Tầng ứng dụng — 1 thư mục / app
│   ├── App_Manager/       #   Điều phối: nhận SELECT_APP từ ESP, vòng đời Init/Run/Deinit, nhận file về SD
│   ├── App_Clock/         #   App mặc định: giờ/lịch (soft RTC từ NTP của ESP) + thời tiết
│   ├── App_Video/         #   Phát lặp VIDEO.RVD từ thẻ SD, tự retry khi chưa có file
...
```
(Tài liệu — bộ file `0x_*.md` này — nằm ở **thư mục gốc repo DesktopAssistant**, cạnh `MainECU_STM32/` và `README.md`.)

Quy tắc: **mỗi module một thư mục riêng**, tên thư mục = tên file = `<Tầng>_<Module>` (ví dụ [Libs/Libs_Spi/Libs_Spi.c](MainECU_STM32/Libs/Libs_Spi/Libs_Spi.c)).
...
- **Tách phần cứng khỏi logic**: đổi chân, đổi peripheral, hoặc port sang MCU khác chỉ cần sửa tầng Wrappers và [main.h](MainECU_STM32/Core/Inc/main.h); tầng Libs và ứng dụng không đổi.
...
1. **Hàm pass-through** — khi cần giữ chỗ để sau này thêm logic:
   ...
   ```
2. **Macro alias** — khi chắc chắn chỉ chuyển tiếp thuần túy (tránh tốn call overhead), như trong [Libs_Gpio.h](MainECU_STM32/Libs/Libs_Gpio/Libs_Gpio.h):
   ```c
...
Toàn bộ mapping tài nguyên vật lý được tập trung ở [main.h](MainECU_STM32/Core/Inc/main.h):
...
- Với **GPIO**: macro `LogicalChannel_x` chứa sẵn dấu phẩy nên 1 "tham số" khi expand thành 2 tham số của `HAL_GPIO_WritePin`. Vì vậy `Wrappers_Gpio_Write` bắt buộc là macro (xem [Wrappers_Gpio.h](MainECU_STM32/Wrappers/Wrappers_Gpio/Wrappers_Gpio.h)), không thể là hàm.
- Với **SPI**: kênh là `uint8_t`, [Wrappers_Spi.c](MainECU_STM32/Wrappers/Wrappers_Spi/Wrappers_Spi.c) chọn handle `SPI1`/`SPI2` bằng if/else theo giá trị kênh.
...
int main(void)                       // CubeMX skeleton
{
    ...
    App_Manager_Init();              // mount SD + EspLink + app mặc định (Clock)
    /* USER CODE END 2 */
    while (1)
    {
        /* USER CODE BEGIN 3 */
        App_Manager_Run();           // lệnh từ ESP + chạy app hiện hành
...
Module lớn nhất và là ứng dụng chính hiện tại, gồm 3 file trong [Libs/Libs_Screen_ILI9341/](MainECU_STM32/Libs/Libs_Screen_ILI9341/):

| File | Vai trò |
|---|---|
| [Libs_ILI9341_Driver.c/h](MainECU_STM32/Libs/Libs_Screen_ILI9341/Libs_ILI9341_Driver.c) | Tầng thấp: reset, chuỗi lệnh init, set address window, set rotation, fill screen, draw pixel/rectangle/line. Giao tiếp LCD qua `Wrappers_Spi_Transmit` + `Wrappers_Gpio_Write` (CS/DC/RST) |
| [Libs_ILI9341_GFX.c/h](MainECU_STM32/Libs/Libs_Screen_ILI9341/Libs_ILI9341_GFX.c) | Tầng vẽ: hình tròn (rỗng/đặc), chữ nhật theo tọa độ, ký tự / chuỗi (font 6×8, scale được), ảnh full-screen RGB565 |
| [Libs_ILI9341_Font.h](MainECU_STM32/Libs/Libs_Screen_ILI9341/Libs_ILI9341_Font.h) | Font bitmap 96 ký tự ASCII (từ ' '), mảng `g_Font_aa[96][6]` |
...
- Thẻ SD qua FatFs: driver SPI-mode [Libs_SdCard](MainECU_STM32/Libs/Libs_SdCard/Libs_SdCard.h) + `user_diskio.c` (mount / đọc / ghi file).
- App test phần cứng [App_HwTest](MainECU_STM32/App/App_HwTest/App_HwTest.h) — app hiện hành chạy trong `while(1)` của main.c.
...
| `Tools/video2c.py` | Script convert video được nhắc trong [Libs_Video.h](MainECU_STM32/Libs/Libs_Video/Libs_Video.h) nhưng **chưa có trong repo** — cần bổ sung để tạo dữ liệu frame/.rvd |
| SD card | **Đã implement** ([Libs_SdCard](MainECU_STM32/Libs/Libs_SdCard/Libs_SdCard.c) + [user_diskio.c](MainECU_STM32/FATFS/Target/user_diskio.c)). Đọc/ghi theo đơn block (CMD17/CMD24) — đủ cho test và file nhỏ; khi stream video từ SD nên nâng cấp multi-block (CMD18/CMD25). Có thể bật `VIDEO_USE_SDCARD=1` trong Libs_Video.h |
...
| PWM | Module tên là Pwm nhưng [Wrappers_Pwm.c](MainECU_STM32/Wrappers/Wrappers_Pwm/Wrappers_Pwm.c) mới init **TIM2 base timer**, chưa cấu hình PWM output channel |
...
## 8. Cơ chế App + điều khiển từ ESP Station (thêm 2026-08-31)

```
ESP Station ──UART frame──▶ Libs_EspLink ──▶ App_Manager ──▶ App hiện hành
   (Web UI)   (StationLinkCore)  │ tự trả ACK      │ bảng app id = LINK_APP_*
                                 │ soft RTC        │ Init/Run()/Deinit
                                 │ thời tiết       │ 0=Clock 1=Video 2=HwTest
                                 └ FILE_* ─────────┴─▶ callback ghi SD + CRC32
```

- **Đổi app**: Web UI của Station → `SELECT_APP` → EspLink validate + ACK → App_Manager
  Deinit app cũ, Init app mới. App id là hợp đồng chung trong `StationLinkCore.h`.
- **App mới phải có** `App_X_Init/Run` (Run **không blocking dài** — mọi app chạy chung
  1 vòng while với EspLink + blink), `Deinit` tùy chọn (Video cần để đóng file).
  Thêm app = thêm 1 dòng vào `l_AppTable_st` (App_Manager.c) + id mới ở cả 2 phía protocol.
- **Giờ/lịch**: ESP đẩy `TIME_SYNC` (NTP + múi giờ) → EspLink giữ soft RTC bằng tick;
  `App_Clock_EpochToCivil` đổi epoch→lịch (đã đối chiếu 1007 mốc với `gmtime` trên host).
- **Nhận file**: FILE_START/DATA/END → App_Manager ghi thẳng xuống SD, chống trùng khối
  (ESP retransmit khi mất ACK), CRC32 toàn file khớp mới báo OK. FatFs đang cấu hình tên 8.3.
- **Bản sao protocol**: `Libs/Libs_EspLink/StationLinkCore.[ch]` phải giữ đồng bộ với
  `Station/src/Link/` — sửa một nơi thì diff nơi kia (ghi chú ngay đầu file).
